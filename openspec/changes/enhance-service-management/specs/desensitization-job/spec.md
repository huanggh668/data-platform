...
### Requirement: Desensitization job CRUD operations
The system SHALL allow users to create, read, update, and delete desensitization jobs.

#### Scenario: Create desensitization job
- **WHEN** user fills in job name, selects source, target, and rules, then clicks "Create"
- **THEN** system creates job with PENDING status and returns success message

#### Scenario: List desensitization jobs
- **WHEN** user navigates to job management page
- **THEN** system displays paginated list of jobs with name, status, execute time, and completion time

#### Scenario: Delete desensitization job
- **WHEN** user clicks "Delete" on a job
- **THEN** system performs logical deletion and removes from list

### Requirement: Job execution
The system SHALL support executing desensitization jobs manually.
...
- **THEN** system displays job details including source, target, rules, and execution result

#### Scenario: Job status states
- **WHEN** job is in any state (PENDING, RUNNING, COMPLETED, FAILED)
- **THEN** system displays appropriate status badge with distinct color
